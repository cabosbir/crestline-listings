...
const CozbiLandingPage = () => {
  ...
  const canonicalUrl = 'https://www.bircabo.com/cozbi';
  ...
  const getInitialPage = () => {
    if (typeof window !== 'undefined') {
      ...
      if (returning === 'true') {
        const savedState = sessionStorage.getItem('cozbiBrowseState');
        if (savedState) {
          try {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      sessionStorage.setItem('cozbiBrowseState', JSON.stringify(browseState));
    }
  ...
  useEffect(() => {
    const loadMyListings = async () => {
      ...
      try {
        ...
        const cacheKey = `cozbi-my-listings-auto-v${CACHE_VERSION}`;
        const cacheTimeKey = `${cacheKey}-time`;
        const cached = localStorage.getItem(cacheKey);
        const cachedTime = localStorage.getItem(cacheTimeKey);
        ...
        // Search for Cozbi Sanchez OR Baja International Realty agents
        const agentListings = mlsData.filter(listing => {
          ...
          // Match Cozbi specifically
          const nameMatch = listAgentName.toLowerCase().includes('sanchez') || 
                           listAgentName.toLowerCase().includes('cozbi');
