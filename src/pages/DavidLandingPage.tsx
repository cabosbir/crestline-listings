...
const DavidLandingPage = () => {
  ...
  const getInitialTab = () => {
    ...
    return false; // Default to "featured" for David
  };
  ...
  useEffect(() => {
    const loadFeaturedListings = async () => {
      ...
      try {
        const cacheKey = 'david-featured-api-data-v2';
        const cacheTimeKey = `${cacheKey}-time`;
        const cached = localStorage.getItem(cacheKey);
        const cachedTime = localStorage.getItem(cacheTimeKey);
  ...
  useEffect(() => {
    const loadMyListings = async () => {
      ...
      try {
        ...
        const cacheKey = `david-my-listings-auto-v${CACHE_VERSION}`;
        const cacheTimeKey = `${cacheKey}-time`;
        const cached = localStorage.getItem(cacheKey);
        const cachedTime = localStorage.getItem(cacheTimeKey);
        ...
        // Search for David Scott Piper OR Baja International Realty agents
        const agentListings = mlsData.filter(listing => {
          ...
          // Match David specifically
          const nameMatch = listAgentName.toLowerCase().includes('piper') || 
                           listAgentName.toLowerCase().includes('david scott');
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="order-1 lg:order-2 text-center lg:text-left">
              <p className="text-lg mb-2 font-medium" style={{ color: '#d4af37' }}>Your International Real Estate Expert</p>
