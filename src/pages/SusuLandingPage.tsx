...
const SusuLandingPage = () => {
  ...
  const getInitialTab = () => {
    ...
    return false; // Default to "featured" for Susu
  };
  ...
  useEffect(() => {
    const loadFeaturedListings = async () => {
      ...
      try {
        const cacheKey = 'susu-featured-api-data-v2';
        const cacheTimeKey = `${cacheKey}-time`;
        const cached = localStorage.getItem(cacheKey);
        const cachedTime = localStorage.getItem(cacheTimeKey);
  ...
  useEffect(() => {
    const loadMyListings = async () => {
      ...
      try {
        ...
        const cacheKey = `susu-my-listings-auto-v${CACHE_VERSION}`;
        const cacheTimeKey = `${cacheKey}-time`;
        const cached = localStorage.getItem(cacheKey);
        const cachedTime = localStorage.getItem(cacheTimeKey);
        ...
        // Search for Susu Vieira OR Baja International Realty agents
        const agentListings = mlsData.filter(listing => {
          ...
          // Match Susu specifically
          const nameMatch = listAgentName.toLowerCase().includes('vieira') || 
                           listAgentName.toLowerCase().includes('susu');
