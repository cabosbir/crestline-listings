...
const agent = {
  ...
  yearsExperience: 15,
  propertiesSold: 150,
  bio: "Professional with extensive experience in administrative tasks and organizational skills. With solid knowledge of document management and customer service, Erika brings a unique perspective to real estate in Cabo San Lucas.",
  bio2: "Erika has built her career on a foundation of exceptional client service and meticulous attention to detail. Her background in administrative management and HR positions her perfectly to guide clients through the complex process of buying or selling property in Los Cabos.",
  bio3: "With over 15 years of professional experience in various administrative and management roles, including positions at Hotel Esperanza Resort and Hotel Villa del Palmar, Erika understands the luxury hospitality market that defines Cabo San Lucas.",
  bio4: "Her expertise in contract management, payroll administration, and client relations translates seamlessly into real estate, where she excels at managing transactions, coordinating with multiple parties, and ensuring every detail is handled professionally.",
  bio5: "Born and raised in La Paz, Baja California Sur, Erika has deep roots in the region and brings invaluable local knowledge to her clients. She is committed to helping buyers and sellers achieve their real estate goals with professionalism, integrity, and personalized service.",
  certifications: ["Real Estate Agent", "AMPI® Member", "Administrative Management Specialist", "Client Relations Expert"],
...
const fallbackListings = [
  {
    ...
    mlsNumber: "24-4467",
    link: "https://www.flexmls.com/share/D0rH7/",
  },
  {
    id: 2,
    ...
    sqft: "372.06 m²",
    mlsNumber: "25-1679",
    link: "https://www.flexmls.com/share/D0rHM/",
  },
  {
    id: 3,
...
const testimonials = [
  {
    name: "Sarah M.",
    text: "Erika's attention to detail and professionalism made our home buying experience seamless. She understood exactly what we were looking for and guided us through every step of the process.",
    rating: 5
  },
  {
    name: "Roberto & Maria T.",
    text: "Working with Erika was a pleasure. Her organizational skills and dedication to client service are unmatched. She made selling our property stress-free and efficient.",
    rating: 5
  },
  {
    name: "James P.",
    text: "Erika's local knowledge and expertise in administrative management really showed throughout our transaction. She handled all the paperwork flawlessly and kept us informed every step of the way.",
    rating: 5
  }
];
...
const ErikaLandingPage = () => {
  ...
  const getInitialPage = () => {
    if (typeof window !== 'undefined') {
      ...
      if (returning === 'true') {
        const savedState = sessionStorage.getItem('erikaGracianoBrowseState');
        if (savedState) {
          try {
  ...
  const getInitialTab = () => {
    ...
    return false; // Default to "featured"
  };
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      sessionStorage.setItem('erikaGracianoBrowseState', JSON.stringify(browseState));
    }
  ...
  useEffect(() => {
    const loadFeaturedListings = async () => {
      ...
      try {
        const cacheKey = 'erika-graciano-featured-api-data-v5';
        const cacheTimeKey = `${cacheKey}-time`;
        const cached = localStorage.getItem(cacheKey);
        const cachedTime = localStorage.getItem(cacheTimeKey);
        ...
        const shuffled = getShuffledListings(convertedListings, 'erika-graciano-featured-shuffle-v3');
  ...
  useEffect(() => {
    const loadMyListings = async () => {
      ...
      try {
        ...
        const cacheKey = `erika-graciano-my-listings-auto-v${CACHE_VERSION}`;
        const cacheTimeKey = `${cacheKey}-time`;
        const cached = localStorage.getItem(cacheKey);
        const cachedTime = localStorage.getItem(cacheTimeKey);
        ...
        console.log('🤖 AUTO-DETECTING listings for Erika Graciano');
        ...
        // Search for Erika Graciano OR Baja International Realty
        const agentListings = mlsData.filter(listing => {
          ...
          // Match Erika Graciano specifically
          const nameMatch = listAgentName.toLowerCase().includes('graciano') || 
                           listAgentName.toLowerCase().includes('erika');
        ...
        console.log(`✅ Auto-detected ${agentListings.length} listings for Erika Graciano`);
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="order-1 lg:order-2 text-center lg:text-left">
              <p className="text-lg mb-2 font-medium" style={{ color: '#d4af37' }}>Your Dedicated Real Estate Professional</p>
