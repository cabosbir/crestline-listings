...
const ErikaLandingPage = () => {
  ...
  const getInitialPage = () => {
    if (typeof window !== 'undefined') {
      ...
      if (returning === 'true') {
        const savedState = sessionStorage.getItem('erikaBrowseState');
        if (savedState) {
          try {
  ...
  const getInitialTab = () => {
    ...
    return false; // Default to "featured" for Erika
  };
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      sessionStorage.setItem('erikaBrowseState', JSON.stringify(browseState));
    }
  ...
  useEffect(() => {
    const loadFeaturedListings = async () => {
      ...
      try {
        const cacheKey = 'erika-featured-api-data-v5';
        const cacheTimeKey = `${cacheKey}-time`;
        const cached = localStorage.getItem(cacheKey);
        const cachedTime = localStorage.getItem(cacheTimeKey);
        ...
        const shuffled = getShuffledListings(convertedListings, 'erika-featured-shuffle-v3');
  ...
  useEffect(() => {
    const loadMyListings = async () => {
      ...
      try {
        ...
        const cacheKey = `erika-my-listings-auto-v${CACHE_VERSION}`;
        const cacheTimeKey = `${cacheKey}-time`;
        const cached = localStorage.getItem(cacheKey);
        const cachedTime = localStorage.getItem(cacheTimeKey);
        ...
        // Search for Erika Aispuro OR Baja International Realty agents
        const agentListings = mlsData.filter(listing => {
          ...
          // Match Erika specifically
          const nameMatch = listAgentName.toLowerCase().includes('aispuro') || 
