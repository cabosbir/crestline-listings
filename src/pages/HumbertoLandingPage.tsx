...
const agent = {
  ...
  yearsExperience: 15,
  propertiesSold: 148,
  bio: "Passionate about connecting clients with their dream properties in Los Cabos. Humberto brings dedication, local knowledge, and a client-first approach to every transaction. Specializing in luxury properties, he is committed to making your real estate experience in Cabo San Lucas seamless and rewarding.",
...
const testimonials = [
  {
    name: "Carlos & Ana Martínez",
    text: "Humberto was incredibly attentive and knowledgeable. He helped us find the perfect luxury property in Cabo and made the whole process stress-free.",
    rating: 5,
  },
  {
    name: "James Whitfield",
    text: "Working with Humberto was a great experience. His dedication and professionalism stood out from the start. Highly recommend him!",
    rating: 5,
  },
  {
    name: "Laura & Tom Henderson",
    text: "Humberto's local expertise and commitment to his clients is unmatched. He found us exactly what we were looking for in record time.",
    rating: 5,
  },
];
...
// ==================== MAIN COMPONENT ====================
const HumbertoLandingPage = () => {
  const { toast } = useToast();
  const navigate = useNavigate();
  const location = useLocation();
  const canonicalUrl = 'https://www.bircabo.com/humberto';

  const getInitialPage = () => {
    if (typeof window !== 'undefined') {
      const returning = sessionStorage.getItem('returningFromProperty');
      if (returning === 'true') {
        const savedState = sessionStorage.getItem('humbertoBrowseState');
        if (savedState) {
          try {
            const state = JSON.parse(savedState);
            if ((Date.now() - state.timestamp) < 30 * 60 * 1000 && state.url === window.location.pathname) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      sessionStorage.setItem('humbertoBrowseState', JSON.stringify({
    ...
    if (typeof window !== 'undefined') {
      ...
      if (returning === 'true') {
        ...
        if (savedState) {
          try {
            const state = JSON.parse(savedState);
            if (state.url === window.location.pathname && (Date.now() - state.timestamp) < 30 * 60 * 1000) {
              setTimeout(() => {
  ...
  useEffect(() => {
    const loadFeaturedListings = async () => {
      ...
      try {
        const cacheKey = 'humberto-featured-api-data-v1';
        const cacheTimeKey = `${cacheKey}-time`;
        const cached = localStorage.getItem(cacheKey);
        const cachedTime = localStorage.getItem(cacheTimeKey);
        const now = Date.now();
        const threeHours = 3 * 60 * 60 * 1000;
        if (cached && cachedTime && (now - parseInt(cachedTime)) < threeHours) {
          setFeaturedListings(JSON.parse(cached));
          setIsLoadingFeatured(false);
          return;
        }
        const mlsData = await fetchListings({ limit: 50 });
        const convertedListings = mlsData.map(convertMLSToPropertyCard);
        const shuffled = getShuffledListings(convertedListings, 'humberto-featured-shuffle-v1');
        try {
          localStorage.setItem(cacheKey, JSON.stringify(shuffled));
          localStorage.setItem(cacheTimeKey, now.toString());
        } catch (e) {}
  ...
  useEffect(() => {
    const loadMyListings = async () => {
      ...
      try {
        const cacheKey = 'humberto-my-listings-v1';
        const cacheTimeKey = `${cacheKey}-time`;
        const cached = localStorage.getItem(cacheKey);
        const cachedTime = localStorage.getItem(cacheTimeKey);
        const now = Date.now();
        ...
        const mlsData = await fetchListings({ limit: 500, city: 'Cabo San Lucas' });
        const agentListings = mlsData.filter(listing => {
          ...
          const nameMatch = listAgentName.toLowerCase().includes('escoto') || listAgentName.toLowerCase().includes('humberto');
  ...
  return (
    <div className="min-h-screen">
      <Helmet>
        <title>Humberto Escoto - Real Estate Advisor | Luxury Properties | Baja International Realty</title>
        <meta
          name="description"
          content="Connect with Humberto Escoto, Real Estate Advisor specializing in Luxury Properties in Cabo San Lucas. Dedicated, knowledgeable, and client-focused."
        />
        <link rel="canonical" href={canonicalUrl} />
        <meta property="og:title" content="Humberto Escoto - Cabo San Lucas Luxury Real Estate Advisor" />
        <meta property="og:description" content="Specializing in luxury properties. Dedicated real estate advisor at Baja International Realty." />
        <meta property="og:url" content={canonicalUrl} />
        <meta property="og:image" content="https://www.bircabo.com/fotos%20de%20agentes/humberto-escoto.jpeg" />
        ...
        <script type="application/ld+json">
          {JSON.stringify({
            ...
            "email": "humberto@bircabo.com",
            "telephone": "+52 624 147 7708",
            "image": "https://www.bircabo.com/fotos%20de%20agentes/humberto-escoto.jpeg",
            "url": canonicalUrl,
            "knowsAbout": ["Real Estate", "Luxury Properties", "Cabo San Lucas", "Los Cabos"],
          })}
        </script>
      </Helmet>
      <Navbar />
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-8">
            ...
            <div className="flex justify-center gap-2 mb-8">
              <Button variant={showMyListings ? "luxury" : "outline"} onClick={() => setShowMyListings(true)}>
                My Listings ({myListings.length})
              </Button>
              <Button variant={!showMyListings ? "luxury" : "outline"} onClick={() => setShowMyListings(false)}>
          ...
          {(isLoadingFeatured && !showMyListings) || (isLoadingMyListings && showMyListings) ? (
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <p className="text-lg text-muted-foreground">
                {showMyListings ? 'Loading agent listings...' : 'Loading featured properties...'}
              </p>
            </div>
          ) : (
            <>
...
export default HumbertoLandingPage;
