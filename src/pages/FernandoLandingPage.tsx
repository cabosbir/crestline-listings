...
const testimonials = [
  {
    ...
    text: "Fernando's dedication and attention to detail reminded us of a champion athlete. He listened to our needs and found us the perfect vacation home. His positive energy and professionalism made the entire process enjoyable!",
    rating: 5
  },
  {
    name: "Robert & Linda Martinez",
    text: "Working with Fernando was fantastic! His background as a professional tennis player shows in his work ethic—focused, determined, and always giving 100%. He made us feel comfortable and supported throughout our home search.",
    rating: 5
  },
  {
    name: "David Chen",
    text: "Fernando combines the discipline of an athlete with genuine care for his clients. He took time to understand what we were looking for and exceeded our expectations. Highly recommend for anyone buying in Cabo!",
    rating: 5
  }
];
...
const FernandoLandingPage = () => {
  ...
  const getInitialPage = () => {
    if (typeof window !== 'undefined') {
      ...
      if (returning === 'true') {
        const savedState = sessionStorage.getItem('fernandoBrowseState');
        if (savedState) {
          try {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      sessionStorage.setItem('fernandoBrowseState', JSON.stringify(browseState));
    }
    ...
    const loadMyListings = async () => {
      ...
      try {
        ...
        const agentListings = mlsData.filter(listing => {
          ...
          const nameMatch = listAgentName.toLowerCase().includes('fernando cabrera') || 
                           listAgentName.toLowerCase().includes('fernando') && listAgentName.toLowerCase().includes('cabrera') ||
                           listAgentName.toLowerCase() === 'fernando cabrera';
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-8">
            <p className="uppercase tracking-wider mb-2 font-medium" style={{ color: '#d4af37' }}>
              {showMyListings ? 'Baja International Realty Portfolio' : `${agent.agency} Listings`}
            </p>
...
export default FernandoLandingPage;
