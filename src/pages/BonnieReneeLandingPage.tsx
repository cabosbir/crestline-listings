...
const agent = {
  ...
  bio4: "Throughout my career, I've built a reputation for integrity, market knowledge, and personalized service. I understand that buying or selling property is one of life's most significant decisions, and I'm committed to making the process smooth, transparent, and successful for every client.",
  bio5: "Whether you're looking to invest in Cabo San Lucas, find your perfect beachfront retreat, or explore commercial opportunities, I bring the experience and dedication needed to turn your real estate goals into reality.",
  certifications: ["Senior Real Estate Advisor", "AMPI® Member", "Bilingual Real Estate Specialist", "MLS Member"],
...
const testimonials = [
  {
    ...
    text: "Bonnie's deep knowledge of both California and Mexico made our transition seamless. Her bilingual expertise and cultural understanding were invaluable in finding our dream vacation home in Cabo.",
    rating: 5
  },
  {
    name: "David R.",
    text: "With 25 years of experience, Bonnie knows the Baja market inside and out. She guided us through our investment property purchase with professionalism and patience. Highly recommended!",
    rating: 5
  },
  {
    name: "Jennifer L.",
    text: "Bonnie's passion for Mexico and real estate shines through in everything she does. She helped us find the perfect beachfront property and made the entire process enjoyable and stress-free.",
    rating: 5
  }
];
...
const canonicalUrl = 'https://www.bircabo.com/bonnie-renee';
  ...
  const getInitialPage = () => {
    if (typeof window !== 'undefined') {
      ...
      if (returning === 'true') {
        const savedState = sessionStorage.getItem('bonnieReneeBrowseState');
        if (savedState) {
          try {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      sessionStorage.setItem('bonnieReneeBrowseState', JSON.stringify(browseState));
    }
  ...
  useEffect(() => {
    const loadFeaturedListings = async () => {
      ...
      try {
        const cacheKey = 'bonnie-renee-featured-api-data-v1';
        const cacheTimeKey = `${cacheKey}-time`;
        const cached = localStorage.getItem(cacheKey);
        const cachedTime = localStorage.getItem(cacheTimeKey);
  ...
  useEffect(() => {
    const loadMyListings = async () => {
      ...
      try {
        ...
        const cacheKey = `bonnie-renee-my-listings-auto-v${CACHE_VERSION}`;
        const cacheTimeKey = `${cacheKey}-time`;
        const cached = localStorage.getItem(cacheKey);
        const cachedTime = localStorage.getItem(cacheTimeKey);
        ...
        console.log('🤖 AUTO-DETECTING listings for Bonnie Renee G.');
        ...
        // Search for Bonnie Renee OR Baja International Realty
        const agentListings = mlsData.filter(listing => {
          ...
          // Match Bonnie Renee specifically
          const nameMatch = listAgentName.toLowerCase().includes('bonnie') || 
                           listAgentName.toLowerCase().includes('renee');
        ...
        console.log(`✅ Auto-detected ${agentListings.length} listings for Bonnie Renee G.`);
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="order-1 lg:order-2 text-center lg:text-left">
              <p className="text-lg mb-2 font-medium" style={{ color: '#d4af37' }}>Your Trusted Real Estate Partner</p>
...
export default BonnieReneeLandingPage;
