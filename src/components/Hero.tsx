    ...
    <div className="relative container mx-auto px-4 sm:px-6 max-w-6xl">
      ...
      <nav aria-label="What would you like to do?" className="mt-7 sm:mt-9 grid grid-cols-1 sm:grid-cols-6 gap-3 sm:gap-4">
        <a href="/property-search.html" className="sm:col-span-4 flex items-center justify-between gap-4 rounded-xl bg-white p-5 sm:p-7 text-blue-950 shadow-lg hover:bg-blue-50 focus-visible:outline focus-visible:outline-4 focus-visible:outline-amber-400">
          <span className="flex items-center gap-3 sm:gap-4 min-w-0"><Search className="hidden sm:block h-8 w-8 shrink-0" aria-hidden="true" /><span><span className="block text-2xl sm:text-3xl font-bold">Search Properties</span><span className="block mt-2 text-base sm:text-lg leading-relaxed">Search the full MLS inventory available for public search in Los Cabos and Baja California Sur. Save favorites and compare at your own pace—<strong>no signup required.</strong></span></span></span>
